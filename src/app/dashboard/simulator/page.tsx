...
export default function TransactionSimulatorPage() {
  ...
  const [mode, setMode] = useState<CustomerMode>("real");
  ...
  const canSubmit = mode === "synthetic" || !!loadedCustomerId;
  const isRunning = runState.isLoading;
  ...
  async function handleRun() {
    ...
    setRunError(null);
    try {
      const live = await runSimulation(request).unwrap();
  ...
  if (!canUse) return <PermissionDenied />;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200">
          ● Internal tool — no live side effects
        </span>
      </div>
        ...
        <div className={cardCls}>
          <div className="px-5 py-4 border-b border-gray-100 dark:border-navy-600">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Borrow a real customer&apos;s history, or make one up
            </p>
          </div>
